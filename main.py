...
def index():
    ...
    <form method="post" action="/course_edit" >
        <h3>新增與刪除課程</h3>
...
def action(my_head: str = Form(default='')):
    ...
    if check == 0:
        ...
        results += "<table><tr><th>Section ID</th><th>Section Name</th><th>Semester</th><th>Year</th><th>Department</th><th>Max Quota</th><th>Current Students</th><th>Instructor_id</th></tr>"
...
def add_course_form() -> str:
    ...
    <body>
        <form method="post" action="/add_course" >
            ...
            <input type="number" name="credits" placeholder="Credits"><br>
            <input type="text" name="course_type" placeholder="Type (Elective/Required)"><br>
            <input type="text" name="department_name" placeholder="Department Name"><br>
...
# 新增與刪除課程介面
@app.post('/course_added', response_class=HTMLResponse)
def admin_interface_added():
    return add_course_form()

@app.post('/course_delete', response_class=HTMLResponse)
def admin_interface_delete():
    return remove_course_form()

# 編輯課程介面
@app.post('/course_edit', response_class=HTMLResponse)
def course_edit():
    form = """
    <html>
    <head>
    ...
    <form method="post" action="/course_added" >
        <h3>新增課程</h3>
...
# 新增與刪除課程的資料接收與運行
@app.post('/add_course', response_class=HTMLResponse)
def add_course(course_id: str = Form(...), course_name: str = Form(...), credits: int = Form(...),
               course_type: str = Form(...), department_name: str = Form(...)):
    conn = MySQLdb.connect(host="127.0.0.1", user="hj", passwd="test1234", db="coursesystem")
    cursor = conn.cursor()

    query = "INSERT INTO course (Course_id, Course_name, Credits, Type, Department_name) " \
            "VALUES ('{}', '{}', {}, '{}', '{}');".format(course_id, course_name, credits, course_type, department_name)
    cursor.execute(query)
    conn.commit()

    return f"Course added successfully!<p><a href='/'>Back to Query Interface</a></p>"

@app.post('/remove_course', response_class=HTMLResponse)
def remove_course(course_id: str = Form(...)):
    ...
    return f"Course removed successfully!<p><a href='/'>Back to Query Interface</a></p>"